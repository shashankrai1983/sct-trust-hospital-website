...
export const getLocationBySlug = (
  slug: string,
): LocationRegistryEntry | undefined => {
  return LOCATION_REGISTRY.find((location) => location.slug === slug);
};

export const getHighPriorityLocations = (): LocationRegistryEntry[] => {
  return LOCATION_REGISTRY.filter(
    (location) => location.status === "active" && location.priority === "high",
  );
};
...
export const locationParams = getLocationSlugsForStaticGeneration().map(
  (slug) => ({
    location: slug,
  }),
);
